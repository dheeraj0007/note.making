...
downloadNotes.addEventListener('click', () => {
    ...
    console.log(notesObj);
    let everything = document.getElementById("everything").innerHTML;
    let output = "<div style='font-size:20px'>";
    for (let i = 0; i < notesObj.length; i++) {
        output += `<p>${notesObj[i]}</p>`
    }
    output += "</div>";
    document.getElementById('everything').innerHTML = output;
    window.print();
    document.getElementById('everything').innerHTML = everything;
});